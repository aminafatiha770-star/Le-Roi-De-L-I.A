...
const ToolInterface: React.FC<ToolInterfaceProps> = ({ toolId }) => {
  ...
  const handleProcess = async () => {
    ...
    try {
      if (toolId === 'text-to-image') {
        ...
        if (!images[0]) throw new Error("Image requise.");
        const res = await editImage(images[0], `Action: ${toolId}. Instructions: ${mainPrompt}`);
        setOutputs([res, '', '', '']);
      }
    } catch (e: any) {
      alert("⚠️ " + e.message);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-44">
      <div className="flex flex-col md:row justify-between items-center bg-black/30 backdrop-blur-xl p-10 rounded-[3.5rem] border border-gold/30 shadow-[0_0_40px_rgba(0,0,0,0.5)] gap-6">
      ...
      <div className={`grid ${sideBySide ? 'grid-cols-2' : 'grid-cols-1 lg:grid-cols-2'} gap-12`}>
        ...
        <div className="space-y-8 bg-emerald-900/40 p-12 rounded-[4.5rem] border border-white/10 shadow-2xl">
          ...
          <textarea 
            placeholder="Instructions magiques (ex: 'Rendre plus lumineux', 'Style 4k Ultra', 'Ajouter du doré')..."
          ...
          {toolId === 'view-angle' && (
            <select className="w-full p-6 bg-gold text-mauve font-black rounded-3xl outline-none cursor-pointer border-4 border-white/20">
        ...
        <div className="space-y-8 bg-emerald-900/60 p-12 rounded-[4.5rem] border border-white/10 shadow-2xl">
          ...
          {resultText ? (
            <div className="bg-black/60 p-8 rounded-[3rem] border-2 border-gold/30 text-white font-mono text-sm h-[450px] overflow-auto scrollbar-thin scrollbar-thumb-gold leading-relaxed">
              {resultText}
            </div>
          ) : renderGrid(false)}
          ...
          <div className="text-center">
            <p className="text-[10px] text-white/30 uppercase font-black tracking-[0.2em]">
                Survolez une image pour la <span className="text-gold">Télécharger</span> ou la <span className="text-red-400">Supprimer</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-12 left-1/2 -translate-x-1/2 w-[95%] max-w-3xl z-[100] px-4">
          <button 
            ...
            disabled={loading}
            className={`w-full py-8 btn-royal text-3xl tracking-[0.4em] flex items-center justify-center gap-6 shadow-[0_20px_50px_rgba(0,0,0,0.4)] ${loading ? 'opacity-70 grayscale' : 'hover:scale-105'}`}
          >
            {loading ? (
                <>
